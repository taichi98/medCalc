...
# Lấy tiêu chuẩn từ R và chuyển đổi sang Pandas DataFrame
list_standards = {key: robjects.r['list_standards'][key] for key in robjects.r['list_standards'].names}
...
def calculate_z_scores(data):
    ...
    data['height'] = pd.to_numeric(data['height'], errors='coerce')  # Đảm bảo có chiều cao
...
def compute_z_score(measure, standards, age_in_days, sex):
    # Tính toán z-score cho các phép đo
    return (measure - np.mean(standards)) / np.std(standards)

def compute_wfl(data, standards):
    # Tính toán z-score cho cân nặng chiều dài
    return (data['weight'] / (data['length'] / 100) ** 2 - np.mean(standards)) / np.std(standards)

def compute_wfh(data, standards):
    # Tính toán z-score cho cân nặng chiều cao
    return (data['weight'] / (data['height'] / 100) ** 2 - np.mean(standards)) / np.std(standards)
